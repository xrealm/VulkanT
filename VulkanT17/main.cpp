...
/**
 * 描述符布局和缓冲区，使用资源描述符访问缓冲区和图像资源，描述符布局描述了可以绑定的描述符类型
 * 使用统一的缓冲区对象UBO(uniform buffer objects)
 *
 * 指定一个VkBuffer绑定到一个uniform buffer描述符，需要创建描述符集
 */
...
//            vkCmdDraw(commandBuffers[i], vertices.size(), 1, 0, 0);
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("../../VulkanT17/shaders/vert.spv");
        auto fragShaderCode = readFile("../../VulkanT17/shaders/frag.spv");
        //着色器模型
        ...
        //设置frontFace为逆时针
        rastrerizeCreateInfo.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;
